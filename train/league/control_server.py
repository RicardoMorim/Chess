            ...
            def _serve_dashboard(self):
                ...
                if server.dashboard_dir is None or not server.dashboard_dir.exists():
                    body = (
                        ...
                        b"<title>Chess Trainer Control Server</title>"
                        b"<style>"
                        b"body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;"
                        b"max-width:720px;margin:48px auto;padding:24px;background:#0f1419;color:#e6edf3;"
                        b"border-radius:12px;}"
                        b"h1{color:#7ee787;margin-top:0;}"
                        b"code{background:#1a2028;padding:2px 6px;border-radius:4px;color:#79c0ff;}"
                        b"ul{line-height:1.8;}"
                        b".ok{color:#7ee787;} .warn{color:#f0883e;}"
                        b"</style></head><body>"
                        b"<h1>Chess Trainer Control Server</h1>"
                        b"<p class='warn'>Dashboard assets not found at "
                        b"<code>" + str(server.dashboard_dir or "(not configured)").encode() + b"</code>.</p>"
                        b"<p>The HTTP API is fully available. Endpoints:</p>"
                        b"<ul>"
                        b"<li><code>GET  /api/status</code> &mdash; trainer snapshot</li>"
                        b"<li><code>GET  /api/checkpoints</code> &mdash; list .pt files</li>"
                        b"<li><code>GET  /api/variants</code> &mdash; variants + buffer fill</li>"
                        b"<li><code>GET  /api/modes</code> &mdash; performance presets</li>"
                        b"<li><code>GET  /api/knobs</code> &mdash; hot-settable knobs</li>"
                        b"<li><code>GET  /api/matches</code> &mdash; in-flight matches</li>"
                        b"<li><code>GET  /api/matches/stream</code> &mdash; SSE live events</li>"
                        b"<li><code>POST /api/mode</code> &mdash; <code>{\"mode\":\"boost\"}</code></li>"
                        b"<li><code>POST /api/knobs</code> &mdash; <code>{\"knobs\":{\"BATCH_SIZE\":512}}</code></li>"
                        b"<li><code>POST /api/auto_mode</code> &mdash; <code>{\"enabled\":true}</code></li>"
                        b"<li><code>POST /api/pause</code> &mdash; <code>{\"paused\":true}</code></li>"
                        b"<li><code>POST /api/matches</code> &mdash; <code>{\"type\":\"model\",\"params\":{...}}</code></li>"
                        b"</ul>"
                        b"<p class='ok'>Run the Tkinter dashboard instead: "
                        b"<code>cd train &amp;&amp; python -m league.dashboard_tk</code></p>"
                        b"</body></html>"
                    )
                    self.send_response(200)
